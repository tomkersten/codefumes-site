(function($) {
  $(Screw).bind('loaded.screwunit', function() {
    $('.status').fn({
      display: function() {
        $(this).html(
    ...
    $('.it').fn({
      ...
      run_segment: function() {
        ...
        try {
          ...
          $(this).trigger('finished_run.screwunit');
        } catch(e) {
        ...
        try {
          if (failure) {
            $(this).trigger('failed.screwunit', failure);
          } else {
            $(this).trigger('passed.screwunit');
          }
        } finally {
          $(this).fn('parent').fn('run_afters');
      ...
      enqueue: function() {
        var self = $(this).trigger('enqueued.screwunit');
        $(Screw)
          .queue(function() {
            self.fn('run');
